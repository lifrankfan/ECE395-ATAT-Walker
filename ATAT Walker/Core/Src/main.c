...
int32_t Q2[] = {0, 45, 90, 90, 90, 90, 45, 0};
int32_t Q3[] = {45, 45, 45, 90, 120, 135, 90, 45};
...
//	  if(flag){
				  for(int i = 0; i < 8; i++){
					  CCR_reg = ((QMat[3][i]) / 0.18 + 250);
					  __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_4, CCR_reg);
					  CCR_reg = ((QMat[2][i]) / 0.18 + 250);
					  __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, CCR_reg);
					  HAL_Delay(1000);
				  }
//	  CCR_reg = ((90) / 0.18 + 250);
...
void calc_degree(int32_t angle_end, int motor) {
    ...
    for (int i = 0; i < 100; i++) {
        // Calculate the new CCR value based on current angle
        CCR_reg = currState[motor] / 0.18 + 250; //0.27
